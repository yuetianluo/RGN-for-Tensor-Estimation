# Iterative Low-rank tensor regression via rank-1 projection
...
set.seed(2020)
runtime <- proc.time()
########## Tensor regression via rank-1 projection setting ###########
...
for (ratio.index in 1:length(ratio_total)){
  ...
  for (sig in sig_total){
    ...
    for (i in 1:exp.time){
      RGN_error <- RGN_rankone(hatU1,hatU2,hatU3,Xt,y,A,U1,U2,U3,X,r1, r2, r3, p1, p2, p3, t_max, succ_tol, retrac)
...
file_name <- paste("RGN_tenreg_rankone_prop","n",paste(n_total, sep="", collapse="_"),"p", p, "r", r, "sig", paste(sig_total, sep="", collapse="_"), "iter", t_max, 'retrac', retrac, 'tol', succ_tol, 'simu_num', exp.time,'init', init, sep = "_")
print(final_result)
print(file_name)
