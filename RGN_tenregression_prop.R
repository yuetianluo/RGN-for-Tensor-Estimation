# Iterative Low-rank matrix and tensor regression via sketching
# Study the property of RGN in tensor regression.
...
set.seed(2020)
runtime <- proc.time()
########## Tensor regression setting ###########
# n = 8000, p = 20, size = 5, memory 2.4 G
# n = 4000, p = 50, size =1, meomry 4G, so simu_max = 100, we need introduce batch.
p = c(30); r = c(3); sig_total = c(0, 1e-6);
ratio_total <- c(12,14)
...
for (ratio.index in 1:length(ratio_total)){
  ...
  for (sig in sig_total){
    ...
    for (i in 1:exp.time){
      RGN_error <- RGN(hatU1,hatU2,hatU3,Xt,y,A,U1,U2,U3,X,r1, r2, r3, p1, p2, p3, t_max, succ_tol, retrac)
...
file_name <- paste("RGN_tenregression_prop","n",paste(n_total, sep="", collapse="_"),"p", p, "r", r, "sig", paste(sig_total, sep="", collapse="_"), "iter", t_max, 'retrac', retrac, 'tol', succ_tol, 'simu_num', exp.time,'init', init, sep = "_")
print(final_result)
print(file_name)
